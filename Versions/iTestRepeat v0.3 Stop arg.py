...
VERSION = "0.3.1"  # Added: Stop arg & in-line documentation
# Putched: Hebrew support
...
def show_error(message):
    """Displays a topmost error popup that auto-closes after 10 seconds.
    ...
    tk.Label(root, text=message, font=("Segoe UI", 12), padx=20, pady=20).grid(row=0, column=0, columnspan=2)
    ttk.Button(root, text="Close", command=root.destroy).grid(row=1, column=0, padx=10, pady=10)

    root.after(10000, root.destroy)  # Auto-close after 10 seconds (messagebox doesn't support this)
    root.mainloop()
...
def launch(csv_path, csv_specified):
    ...
    if OPEN: subprocess.Popen([PROGRAM_PATH]) # Lanching the proggram
